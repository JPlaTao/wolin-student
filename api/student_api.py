...
def create_student(
    ...
    logger.info(f"[{request_id}] 创建学生: {new_student_data.stu_name}")
    ...
    try:
        ...
        logger.info(f"[{request_id}] 学生创建成功: ID={new_student['stu_id']}")
        
        return response.ResponseBase(
            data=new_student
        )
    except Exception as e:
        logger.error(f"[{request_id}] 创建学生失败: {str(e)}")
        raise
...
def get_students(
    ...
    request_id = getattr(request.state, 'request_id', 'unknown')
    logger.info(f"[{request_id}] 查询学生: stu_id={stu_id}, stu_name={stu_name}, class_id={class_id}")
    ...
    try:
        ...
        logger.info(f"[{request_id}] 查询完成: 找到 {len(students)} 条记录")
        return response.ListResponse(
            data=students,
            total=len(students)
        )
    except Exception as e:
        logger.error(f"[{request_id}] 查询学生失败: {str(e)}")
        raise
...
def update_student(
    ...
    request_id = getattr(request.state, 'request_id', 'unknown')
    logger.info(f"[{request_id}] 更新学生 ID={stu_id}: {update_data.model_dump(exclude_unset=True)}")
    ...
    try:
        ...
        if not is_update_student:
            logger.warning(f"[{request_id}] 更新失败: 学生 ID={stu_id} 不存在")
            raise HTTPException(status_code=400, detail="没有这个学生")

        logger.info(f"[{request_id}] 学生更新成功: ID={stu_id}")
        return response.ResponseBase(
        ...
        logger.error(f"[{request_id}] 更新学生失败: {str(e)}")
        raise
...
def delete_student(
    ...
    try:
        ...
        logger.error(f"[{request_id}] 删除学生失败: {str(e)}")
        raise
